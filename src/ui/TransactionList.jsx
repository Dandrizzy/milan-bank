    ...
    {userTransactions.reverse().map(t => <Table.Row key={t.id}>
     ...
     <Table.Cell >
      <div className="grid gap-1 text-right">
       <div className={t.type === 'deposit' ? ' text-right text-green-400' : ' text-right text-red-400'}>{t?.type === 'deposit' ? '+' : '-'}{formatCurrency({ value: t.amount, currency: acc.currency })}</div>
       <div className="text-xs text-neutral-400 uppercase">{t.status}</div>
      </div>
     </Table.Cell>
    </Table.Row>)}
   </Table.Body>
